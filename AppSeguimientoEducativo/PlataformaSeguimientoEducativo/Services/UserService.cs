...
public class UserService : IUserService
{
    ...
    public async Task<string> LoginAsync(LoginDto loginDto)
    {
        ...
        var result = _passwordHasher.VerifyHashedPassword(user, user.PasswordHash ?? string.Empty, loginDto.Password);
    ...
    public async Task<StudentDashboardDto> GetStudentDashboardAsync(int userId)
    {
            ...
            Teachers = new List<TeacherInfoDto>
        {
            new TeacherInfoDto
            {
                ...
                SubjectName = course.Teacher?.Subject ?? "Unknown",
                Period = course.AcademicPeriod?.PeriodName ?? "Unknown",
                ...
                Feedback = course.Feedbacks
                    ...
                    .FirstOrDefault() ?? new FeedbackDetailDto { FeedbackText = "Sin comentarios", FeedbackDate = "" }
            }
        }
        }).ToList();
    ...
    {
        var result = _passwordHasher.VerifyHashedPassword(user, user.PasswordHash ?? string.Empty, changePasswordDto.CurrentPassword);
        if (result == PasswordVerificationResult.Failed)
        {
